...
import { ArrowUpRight, ArrowDownRight, Users, DollarSign, Activity, Ticket, Star, Shield } from "lucide-react";
import {
  BarChart,
  Bar,
  XAxis,
...
export default function FootballDashboard() {
  ...
  const mergedChartData = useMemo(() => {
    return team.historicalRevenue.map(d => ({ ...d, [team.name + ' Rec']: d.revenue, [team.name + ' Desp']: d.expenses }));
  }, [team]);
    ...
    <div className="h-screen w-full overflow-hidden bg-[#0a0a0a] text-neutral-50 flex flex-col font-sans selection:bg-neutral-800">
      ...
      <main className="flex-1 p-2 md:p-3 flex flex-col gap-2 md:gap-3 overflow-hidden">
        ...
        <div className="flex-none grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 h-24">
          ...
          {renderKPI("Sócio Torcedor", team.members, "", "", <Users className="h-4 w-4 text-blue-500" />)}
          {renderKPI("Público Pagante", team.attendance, "", "", <Ticket className="h-4 w-4 text-amber-500" />)}
        </div>

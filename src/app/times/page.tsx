...
export default function FootballDashboard() {
  ...
  const renderKPI = (title: string, data: any, prefix = "", suffix = "", icon: React.ReactNode, delay: number) => {
    ...
    return (
      <motion.div
        ...
        className="h-full"
      >
        <Card className="flex flex-col bg-neutral-900/50 border-neutral-800 backdrop-blur-sm h-full justify-between p-3 lg:p-4 relative overflow-hidden group">
          ...
          <div className="flex flex-row items-center justify-between pb-1 lg:pb-2">
            <span className="text-[10px] lg:text-xs font-semibold text-neutral-400">{title}</span>
          ...
          <div className="mt-1 flex flex-col justify-end flex-1">
            <motion.div 
              ...
              className="text-lg lg:text-xl font-black text-white tracking-tight"
            >
              {prefix}{data.current.toLocaleString("pt-BR")}{suffix}
            </motion.div>
            <div className="flex gap-1 lg:gap-2 mt-2 lg:mt-3 text-[9px] lg:text-[10px] font-medium">
              <Badge variant="outline" className={isMomPositive ? "text-emerald-400 bg-emerald-400/10 border-emerald-400/20 px-1 py-0" : "text-rose-400 bg-rose-400/10 border-rose-400/20 px-1 py-0"}>
                {isMomPositive ? <ArrowUpRight className="mr-0.5 h-2.5 w-2.5" /> : <ArrowDownRight className="mr-0.5 h-2.5 w-2.5" />}
              ...
              <Badge variant="outline" className={isYoyPositive ? "text-emerald-400 bg-emerald-400/10 border-emerald-400/20 px-1 py-0" : "text-rose-400 bg-rose-400/10 border-rose-400/20 px-1 py-0"}>
                {isYoyPositive ? <ArrowUpRight className="mr-0.5 h-2.5 w-2.5" /> : <ArrowDownRight className="mr-0.5 h-2.5 w-2.5" />}
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-[#050505] text-neutral-50 flex flex-col font-sans selection:bg-neutral-800">
      ...
      {/* Header Premium */}
      <header 
        className="flex-none px-4 py-2 lg:px-6 lg:py-3 flex items-center justify-between border-b border-white/5 relative"
      >
        <div 
        ...
        <div className="flex items-center gap-3 w-1/3">
          <motion.div 
            ...
            className="w-10 h-10 lg:w-12 lg:h-12 bg-white/10 backdrop-blur-xl rounded-xl p-1.5 flex items-center justify-center border border-white/10 relative shadow-2xl overflow-hidden"
          >
            ...
            />
            <div className="absolute -top-1.5 -right-1.5 bg-neutral-900 border border-neutral-700 text-[9px] lg:text-[10px] font-black w-5 h-5 lg:w-5 lg:h-5 rounded-full flex items-center justify-center z-20 shadow-xl">
              {team.position}
            </div>
          </motion.div>
          <div>
            <motion.h1 
              ...
              className="text-base lg:text-lg font-black tracking-tight text-white leading-none mb-0.5"
            >
              {team.name}
            </motion.h1>
            <div className="flex gap-2 items-center">
              <Badge variant="secondary" className="text-[8px] lg:text-[9px] px-1.5 py-0 bg-neutral-800 text-neutral-300 border-neutral-700 hover:bg-neutral-700">
                Série A
              </Badge>
              <span className="text-neutral-500 text-[9px] lg:text-[10px] font-medium">Dashboard Executivo</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 justify-center w-1/3">
          <Select value={year} onValueChange={(val) => setYear(val || "2024")}>
            <SelectTrigger className="w-[70px] lg:w-[90px] h-7 lg:h-8 bg-neutral-900/50 border-neutral-800 text-[10px] lg:text-xs text-neutral-300 focus:ring-1 focus:ring-neutral-700">
          ...
          <Select value={month} onValueChange={(val) => setMonth(val || "Todos")}>
            <SelectTrigger className="w-[80px] lg:w-[100px] h-7 lg:h-8 bg-neutral-900/50 border-neutral-800 text-[10px] lg:text-xs text-neutral-300 focus:ring-1 focus:ring-neutral-700">
          ...
          <Select value={championship} onValueChange={(val) => setChampionship(val || "Todos")}>
            <SelectTrigger className="w-[100px] lg:w-[120px] h-7 lg:h-8 bg-neutral-900/50 border-neutral-800 text-[10px] lg:text-xs text-neutral-300 focus:ring-1 focus:ring-neutral-700">
        ...
        <div className="flex items-center justify-end w-1/3">
          <div className="bg-neutral-900/80 p-0.5 lg:p-1 rounded-lg border border-neutral-800 flex items-center">
            <Select value={selectedTeamId} onValueChange={(val) => val && setSelectedTeamId(val)}>
              <SelectTrigger className="w-[150px] lg:w-[180px] h-7 lg:h-8 bg-transparent border-0 shadow-none focus:ring-0 text-[10px] lg:text-xs font-semibold">
                  ...
                  <SelectItem key={t.id} value={t.id} className="cursor-pointer py-1.5 focus:bg-neutral-800">
                    <div className="flex items-center gap-2 lg:gap-3">
                      ...
                      <img 
                        ...
                        onError={(e) => { e.currentTarget.style.display = 'none'; }}
                      />
                      <span className="font-bold text-[10px] lg:text-xs truncate">{t.name}</span>
                    </div>
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      {/* Main Content Area */}
      <main className="flex-1 p-2 md:p-3 flex flex-col gap-2 md:gap-3 overflow-hidden relative">
        ...
        <div className="absolute inset-0 z-0 flex items-center justify-center opacity-[0.03] pointer-events-none transition-all duration-1000">
          ...
          <img 
            src={team.logo} 
            alt="" 
            className="w-full h-[150%] max-w-[600px] max-h-[600px] object-contain grayscale" 
            onError={(e) => { e.currentTarget.style.display = 'none'; }}
          />
        </div>

        {/* KPIs Row */}
        <div className="flex-none grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-3 h-20 lg:h-24 z-10 relative">
          {renderKPI("Receita Bruta (Acumulado)", team.revenue, "R$ ", " M", <DollarSign className="h-4 w-4 text-emerald-400" />, 0.1)}
          {renderKPI("Despesas Operacionais", team.expenses, "R$ ", " M", <Activity className="h-4 w-4 text-rose-400" />, 0.2)}
          {renderKPI("Sócio Torcedor", team.members, "", "", <Users className="h-4 w-4 text-blue-400" />, 0.3)}
          {renderKPI("Média de Público", team.attendance, "", "", <Ticket className="h-4 w-4 text-amber-400" />, 0.4)}
        </div>

        {/* Charts & Players Grid */}
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-2 lg:gap-3 min-h-0 z-10">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-3 flex flex-col">
            <Card className="bg-neutral-900/40 border-neutral-800/80 backdrop-blur-md flex flex-col h-full shadow-xl">
              <CardHeader className="py-2 px-3 flex-none border-b border-neutral-800/50">
                <CardTitle className="text-[10px] lg:text-xs font-black text-white flex items-center gap-1.5">
                  <Shield className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-neutral-400" />
                  Atributos do Clube
                </CardTitle>
              </CardHeader>
              <CardContent className="flex-1 p-1 min-h-0 relative">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="60%" data={radarData}>
                    <PolarGrid stroke="#262626" />
                    <PolarAngleAxis dataKey="subject" tick={{ fill: '#a3a3a3', fontSize: 8, fontWeight: 'bold' }} />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="lg:col-span-6 flex flex-col">
            <Card className="bg-neutral-900/40 border-neutral-800/80 backdrop-blur-md flex flex-col h-full shadow-xl">
              <CardHeader className="py-2 px-3 flex-none border-b border-neutral-800/50">
                <CardTitle className="text-[10px] lg:text-xs font-black text-white flex justify-between items-center">
                  Evolução Financeira (R$ M)
                  <div className="flex gap-2">
                    <div className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full" style={{backgroundColor: team.colors.primary}}></div><span className="text-[8px] lg:text-[9px] text-neutral-400 font-medium">Receita</span></div>
                    <div className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-rose-500"></div><span className="text-[8px] lg:text-[9px] text-neutral-400 font-medium">Despesas</span></div>
                  </div>
                </CardTitle>
              </CardHeader>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={team.historicalRevenue} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                    ...
                    <Tooltip 
                      ...
                      itemStyle={{ color: '#fff' }}
                    />
                    <Bar dataKey="expenses" name="Despesas" fill="#ef4444" radius={[2, 2, 0, 0]} maxBarSize={25} />
                    <Bar dataKey="revenue" name="Receita" fill={team.colors.primary} radius={[2, 2, 0, 0]} maxBarSize={25} />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
          ...
          <div className="lg:col-span-3 flex flex-col gap-2 lg:gap-3 min-h-0">
            {/* Top Players */}
                ...
                <CardContent className="flex-1 p-0 overflow-y-auto min-h-0">
                  <div className="divide-y divide-neutral-800/50">
                    {team.topPlayers?.slice(0, 3).map((player: any, idx: number) => (
                      <div key={player.name + team.id} className="p-2 lg:p-2.5 hover:bg-neutral-800/30 transition-colors flex items-center gap-2">
                        <div className="w-7 h-7 lg:w-8 lg:h-8 rounded-full bg-neutral-800 flex items-center justify-center text-xs font-black text-white border border-neutral-700 shadow-inner">
                          {player.name.charAt(0)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-[10px] lg:text-xs font-bold text-white truncate">{player.name}</p>
                          <p className="text-[8px] text-neutral-500 uppercase tracking-widest font-semibold">{player.pos}</p>
                        </div>
                        <div className="flex items-center gap-1 bg-neutral-950 px-1.5 py-0.5 rounded-md border border-neutral-800">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} className="flex-1 flex flex-col min-h-0">
              <Card className="bg-neutral-900/40 border-neutral-800/80 backdrop-blur-md flex flex-col h-full shadow-xl">
                <CardHeader className="py-2 px-3 flex-none border-b border-neutral-800/50">
                  <CardTitle className="text-[10px] lg:text-xs font-black text-white flex items-center gap-1.5">
                    <PieChartIcon className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-purple-400" />
                    Receitas
                  </CardTitle>
                </CardHeader>
                <CardContent className="flex-1 p-1 min-h-0 relative flex items-center justify-center">
                  ...
                  <div className="absolute top-1/2 -translate-y-1/2 right-2 flex flex-col gap-1">
                    {team.categories?.map((cat: any, i: number) => (
                      <div key={cat.name} className="flex items-center gap-1.5 text-[7px] lg:text-[8px] font-semibold text-neutral-400">
                        <div className="w-1.5 h-1.5 rounded-full shadow-sm" style={{ backgroundColor: COLORS[i % COLORS.length] }} />
                        {cat.name}
                      </div>
                    ))}
                  </div>

      ...
      >
        <Card className="flex flex-col bg-neutral-900/50 border-neutral-800 backdrop-blur-sm h-full justify-between p-3 lg:p-5 relative overflow-hidden group">
          ...
          <div className="flex flex-row items-center justify-between pb-1 lg:pb-2">
            <span className="text-[11px] lg:text-sm font-semibold text-neutral-400">{title}</span>
          ...
          <div className="mt-1 flex flex-col justify-end flex-1">
            ...
            <div className="flex gap-2 mt-2 lg:mt-3 text-[10px] lg:text-xs font-medium">
              <Badge variant="outline" className={isMomPositive ? "text-emerald-400 bg-emerald-400/10 border-emerald-400/20 px-1.5 py-0" : "text-rose-400 bg-rose-400/10 border-rose-400/20 px-1.5 py-0"}>
                {isMomPositive ? <ArrowUpRight className="mr-0.5 h-3 w-3" /> : <ArrowDownRight className="mr-0.5 h-3 w-3" />}
                {Math.abs(mom).toFixed(1)}% MoM
              </Badge>
              <Badge variant="outline" className={isYoyPositive ? "text-emerald-400 bg-emerald-400/10 border-emerald-400/20 px-1.5 py-0" : "text-rose-400 bg-rose-400/10 border-rose-400/20 px-1.5 py-0"}>
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-[#050505] text-neutral-50 flex flex-col font-sans selection:bg-neutral-800">
      ...
      >
        ...
        <div className="flex items-center gap-3 w-1/3">
          ...
          <div>
            ...
            <div className="flex gap-2 items-center">
              <Badge variant="secondary" className="text-[9px] lg:text-[10px] px-1.5 py-0 bg-neutral-800 text-neutral-300 border-neutral-700 hover:bg-neutral-700">
                Série A
              </Badge>
              <span className="text-neutral-500 text-[10px] lg:text-xs font-medium">Dashboard Executivo</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 justify-center w-1/3">
          <Select value={year} onValueChange={(val) => setYear(val || "2024")}>
            <SelectTrigger className="w-[80px] lg:w-[100px] h-8 lg:h-9 bg-neutral-900/50 border-neutral-800 text-[11px] lg:text-sm text-neutral-300 focus:ring-1 focus:ring-neutral-700">
          ...
          <Select value={month} onValueChange={(val) => setMonth(val || "Todos")}>
            <SelectTrigger className="w-[90px] lg:w-[110px] h-8 lg:h-9 bg-neutral-900/50 border-neutral-800 text-[11px] lg:text-sm text-neutral-300 focus:ring-1 focus:ring-neutral-700">
          ...
          <Select value={championship} onValueChange={(val) => setChampionship(val || "Todos")}>
            <SelectTrigger className="w-[110px] lg:w-[140px] h-8 lg:h-9 bg-neutral-900/50 border-neutral-800 text-[11px] lg:text-sm text-neutral-300 focus:ring-1 focus:ring-neutral-700">
        ...
        <div className="flex items-center justify-end w-1/3">
          <div className="bg-neutral-900/80 p-1 lg:p-1.5 rounded-lg border border-neutral-800 flex items-center">
            <Select value={selectedTeamId} onValueChange={(val) => val && setSelectedTeamId(val)}>
              <SelectTrigger className="w-[180px] lg:w-[220px] h-8 lg:h-10 bg-transparent border-0 shadow-none focus:ring-0 text-[11px] lg:text-sm font-semibold">
      ...
      <main className="flex-1 p-3 md:p-6 flex flex-col gap-3 md:gap-5 overflow-hidden relative">
        ...
        {/* Charts & Players Grid */}
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 md:gap-5 min-h-0 z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="lg:col-span-6 flex flex-col">
            <Card className="bg-neutral-900/40 border-neutral-800/80 backdrop-blur-md flex flex-col h-full shadow-xl">
              <CardHeader className="py-3 px-4 flex-none border-b border-neutral-800/50">
                <CardTitle className="text-[11px] lg:text-sm font-black text-white flex justify-between items-center">
                  Evolução Financeira (R$ M)
                  <div className="flex gap-3">
                    <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full" style={{backgroundColor: team.colors.primary}}></div><span className="text-[9px] lg:text-[11px] text-neutral-400 font-medium">Receita</span></div>
                    {/* Fixed Despesas to always be Neutral Gray so it doesn't clash with red teams */}
                    <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-neutral-600"></div><span className="text-[9px] lg:text-[11px] text-neutral-400 font-medium">Despesas</span></div>
                  </div>
                </CardTitle>
              </CardHeader>

...
export default function Home() {
  ...
  const filteredProjects = useMemo(() => {
    if (projectFilter === t.projects.tags.all) return projects;
    ...
    <main className="min-h-screen relative selection:bg-primary/30 overflow-x-hidden flex flex-col items-center">
      ...
      >
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-full font-bold text-xl hover:scale-110 transition-transform"
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-t border-border/50 flex justify-around items-center p-3 shadow-[0_-10px_30px_rgba(0,0,0,0.1)]">
        ...
        <button
          onClick={() => setLang(lang === 'pt' ? 'en' : 'pt')}
          className="flex flex-col items-center gap-1 p-2 text-muted-foreground hover:text-primary transition-colors text-xl"
        >
          {lang === 'pt' ? '🇺🇸' : '🇧🇷'}
          <span className="text-[10px] font-medium">{lang === 'pt' ? 'EN' : 'PT'}</span>
        </button>
      </nav>
      ...
      >
        <div className="space-y-2 text-center flex flex-col items-center">
          <h3 className="text-3xl font-bold tracking-tight inline-flex items-center justify-center gap-3">
            <Briefcase className="w-8 h-8 text-primary" /> {t.projects.title}
          </h3>
          <p className="text-muted-foreground text-center max-w-2xl">{t.projects.description}</p>
        </div>
